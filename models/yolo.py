            ...
            if not self.training or self.export:  # inference                
                if self.export:
                  ...
                  #  self.a = self.anchor_grid_awesome[i]
                    self.a =  torch.tensor(self.anchor_grid[i][...,:]).view( 1, -1, 1, 1, 2)
                    ...
                    x1y1, x2y2, conf,p1,p2,p3,p4,p5, prob = torch.split(y, [2, 2, 1,2,2,2,2,2,self.nc], dim=4)
                    x1y1 = ((x1y1*2. - 0.5 + self.grid[i].to(x_i.device)) * self.stride[i]).type(x_i.dtype)
                    x2y2 = (x2y2 * 2) ** 2 * self.a
                   # y[..., 5:15] = y[..., 5:15] * 8 - 4

                    P1 = (p1 * 8 - 4)* self.a + self.grid[i].to(x[i].device) * self.stride[
                        i].type(x_i.dtype)  # landmark x1 y1
                    P2 = (p2 * 8 - 4)*  self.a + self.grid[i].to(x[i].device) * self.stride[
                        i].type(x_i.dtype)  # landmark x2 y2
                    P3 = (p3 * 8 - 4)*  self.a + self.grid[i].to(x[i].device) * self.stride[
                        i].type(x_i.dtype)  # landmark x3 y3
                    P4 = (p4 * 8 - 4)*  self.a + self.grid[i].to(x[i].device) * self.stride[
                        i].type(x_i.dtype)  # landmark x4 y4
                    P5 = (p5 * 8 - 4)*  self.a + self.grid[i].to(x[i].device) * self.stride[
                        i].type(x_i.dtype)  # landmark x5 y5
                    xyxy = torch.cat((x1y1, x2y2), dim=4)
                    mask = torch.cat((P1, P2, P3, P4, P5), dim=4)
                    # # add a idx (label ids before prob)
                   # idxs = torch.argmax(prob, dim=-1).unsqueeze(axis=-1).type(x_i.dtype).to(x_i.device)
                    y = torch.cat((xyxy,conf,mask ,prob), dim=4).to(x_i.device)
                  #  y = torch.cat((conf, prob), dim=4).to(x_i.device)
